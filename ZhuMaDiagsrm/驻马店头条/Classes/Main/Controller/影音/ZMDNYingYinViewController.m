...
@interface ZMDNYingYinViewController ()

@end

@implementation ZMDNYingYinViewController
- (void)Back {
...
-(instancetype)initWithAddVCARY:(NSArray *)VCS TitleS:(NSArray *)TitleS{
    if (self = [super init]) {
        ...
        //先初始化各个界面
        UIView *BJView  = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kDeviceWidth, 15 + 30)];
        BJView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:BJView];
        
        for (int i = 0 ; i<_JGVCAry.count; i++) {
            UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
            btn.frame = CGRectMake(i*(kDeviceWidth/_JGVCAry.count), 5, kDeviceWidth/_JGVCAry.count, BJView.frame.size.height-2);
