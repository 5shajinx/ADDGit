...
- (void)commonInit {
    ...
    if(_inEditState ) {
        //_inEditState = !_inEditState;
        _undidselctedSectionTitle = @"  点击删除或拖动可自定义频道";

    } else{
    _undidselctedSectionTitle = @"";
    }
    [self addSubview:self.collectionView];
    [self.collectionView addGestureRecognizer:self.panGesture];
    [self.collectionView addGestureRecognizer:self.longPressGesture];
}
//显示hader的文字和刷新界面重新设定前三个的背景图片
- (void)commonInitaddRelodedate{
...
- (void)layoutSubviews {
  ...
  //  ZMDAddViewController *zmdVc = [[ZMDAddViewController alloc]init];
    
//    zmdVc.myblock = ^(){
//        [self commonInitaddRelodedate];
//    };
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(commonInitaddRelodedate) name:@"showUpTitle" object:nil];
    
    
    
    
    self.collectionView.frame = self.bounds;
//    CGPoint dd = _collectionView.center;
//    dd.y += 20;
//    _collectionView.center = dd;
}
...
//                    }
                ...
                // 隐藏新的cell
                cell.alpha = 0.f;
                [_delegate tagView:self SelectTagWithArray:self.selectedItems UnSelectWithArray:self.unselectedItems];
            }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        //前三个不能被选中改变其颜色
        if ((_undidselctedSectionTitle.length >6)) {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        header.titleLabel.text = _undidselctedSectionTitle;
        ...
        if (indexPath.section == 0) {// 删除第一组的
            if (indexPath.row == 0||indexPath.row == 1
              //  ||indexPath.row == 2
                ...
                [self showAlert:@"操作不被允许"];
                
                return; // 不能编辑第一个第二个第三个
            }
            // 添加新数据
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panGesture) {
        if (indexPath) { // indexPath不为nil 说明手指开始的位置是在cell上面
            if (indexPath.section == 0 && indexPath.row != 0 && indexPath.row != 1 && _inEditState) {
...
- (UICollectionView *)collectionView {
    ...
    UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.flowLayout];
        //只执行一次
        CGPoint dd = collectionView.center;
        dd.y += 20;
        collectionView.center = dd;
...
- (UICollectionViewFlowLayout *)flowLayout {
    if (!_flowLayout) {
        ...
        flowLayout.headerReferenceSize = CGSizeMake(70, 30);
