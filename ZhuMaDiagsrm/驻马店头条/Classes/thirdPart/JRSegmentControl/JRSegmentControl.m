...
- (void)createUI
{
    ...
    for (int i = 0; i < _btnCount; i++)
    {
        ...
        /* 测试
        if (i == 2) {
            UIImage *butImage = [UIImage imageNamed:@"
         .png"];
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,20,40,40)];
            [imageView setImage:butImage];
            
            [btn addSubview:imageView];
            [btn setTitle:@"tyuio" forState:UIControlStateNormal];
        }
        */
        if ([[Manager sharedManager].jrStr isEqualToString: @"jrsegment"]) {
