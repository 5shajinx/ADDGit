...
static NSString *MovieCasher = @"MovieCasher";
#define JULI (ScreenW - 320)/3

@interface CustomViewController ()<UITableViewDelegate,UITableViewDataSource,UMSocialUIDelegate>

{
    //NSString *MovieCasher;
...
- (void)viewDidLoad {
   ...
   self.view.backgroundColor = [UIColor whiteColor];
    ...
    //if ([Manager sharedManager].cateid.count != 0) {
    
        self.tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 0*Kscaleh, ScreenW, ScreenH)];
...
- (void)shiting:(NSNotification *)text{
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    NSInteger index = [_text integerValue];
    //if ([Manager sharedManager].cateid.count != 0) {
            INDEX = index;
            //[self request];
    //MovieCasher = [NSString stringWithFormat:@"%ld",index];
    //}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.sharebtn.layer.masksToBounds = YES;
...
//        [cell.imageview sd_setImageWithURL:[NSURL URLWithString:model.ar_pic]];
    ...
    cell.sourcelable.hidden = YES;
    cell.commonlable.hidden = YES;
...
//        [cell.userimageview.layer setBorderWidth:1];
//        [cell.userimageview.layer setBorderColor:[[UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1] CGColor]];
...
- (void)getDataFromlocal {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSString *filewebCaches = [file stringByAppendingPathComponent:MovieCasher];
...
- (void)hhhhhhhhhh:(id)arr{
    ...
    self.recommendArr =  [DetailsModel mj_objectArrayWithKeyValuesArray:arr];
...
- (void)request{
    ...
    if ([Manager sharedManager].cateid.count != 0) {
        currentPage = 0;
        NSDictionary *par = @{@"ar_cateid":[Manager sharedManager].cateid[INDEX],@"ar_id":[NSNumber numberWithInteger:currentPage]};
        ...
        [session GET:DTURL parameters:par progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            ///NSArray *ayyyyy = [DetailsModel mj_objectArrayWithKeyValuesArray:arr];
                      //本地取数据
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                NSString *file = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model.ar_wl isEqual:[NSNull null]] || model.ar_wl == nil || [model.ar_wl isEqualToString:@""]) {
        ...
        self.playerview.commitNumber = [NSString stringWithFormat:@"%@",model.ar_volume];
