...
{
    UIView *pagerBarView = [[UIView alloc]initWithFrame:CGRectMake(0, 20, CGRectGetWidth(self.view.frame), self.contentTopEdging)];
...
- (CGRect)cellFrameWithIndex:(NSInteger)index
{
    ...
    if (index != 0) {
      cellAttrs   = [_collectionViewBar layoutAttributesForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    }
...
- (void)layoutTabPagerView
{
    ...
    if ([Manager sharedManager].lv == 1000) {
        _pagerBarView.frame = CGRectMake(0, 60, CGRectGetWidth(self.view.frame), self.contentTopEdging+[self statusBarHeight]);
    }
    if ([Manager sharedManager].lv == 0) {
        
        _pagerBarView.frame = CGRectMake(0, 20, CGRectGetWidth(self.view.frame), self.contentTopEdging+[self statusBarHeight]);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    if (_barStyle == TYPagerBarStyleProgressBounceView) {
        ...
        NSDictionary *dict =[[NSDictionary alloc] initWithObjectsAndKeys:@"hid",@"btn",nil];
        NSNotification *notification =[NSNotification notificationWithName:@"hidbtn" object:nil userInfo:dict];
